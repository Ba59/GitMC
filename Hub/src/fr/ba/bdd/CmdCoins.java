...
public class CmdCoins implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String msg, String[] args) {
		...
		if(sender instanceof Player){
			...
			if(args.length == 0){
				int balance = SqlConnection.getBalance(p);
				p.sendMessage("§7Tu as §a" + balance+" §7coins.");
			}
			...
			if(args.length >= 1){
				...
				if(args[0].equalsIgnoreCase("add")){
					...
					if(args.length == 1 || args.length == 2){
						p.sendMessage(ChatColor.RED+"La commande est /coins <add>/<remove> [joueur] [montant].");
					}
					...
					if(args.length == 3){
						...
						if(cible != null){	
							...
							SqlConnection.addMoney(cible, montant);
							cible.sendMessage(ChatColor.GRAY+"Tu as reçu "+ ChatColor.GREEN+ montant+ChatColor.GRAY+" coins de la part de " +ChatColor.GREEN+ p.getName()+ChatColor.GRAY+".");
							p.sendMessage(ChatColor.GRAY+"Tu as ajouté "+ ChatColor.GREEN+ montant+ChatColor.GRAY+" coins à " +ChatColor.GREEN+ p.getName()+ChatColor.GRAY+".");
				...
				if(args[0].equalsIgnoreCase("remove")){
					...
					if(args.length == 3){
						...
						if(cible != null){	
							int montant = Integer.valueOf(args[2]);
							SqlConnection.removeMoney(cible, montant);
							cible.sendMessage(ChatColor.GREEN+ p.getName()+ChatColor.GRAY+" t'as retiré "+ ChatColor.GREEN+ montant+ChatColor.GRAY+" coins" +ChatColor.GRAY+".");
							p.sendMessage(ChatColor.GRAY+"Tu as retiré "+ ChatColor.GREEN+ montant+ChatColor.GRAY+" coins à " +ChatColor.GREEN+ p.getName()+ChatColor.GRAY+".");
						}}else { p.sendMessage(ChatColor.RED+"Tu n'as pas accès à cette commande.");}
